...
class TestImplementationCrossChecks:
    ...
    @pytest.mark.xfail(
        reason="Finding H1: calculate_distorted_transient_sync in single-hit mode "
        "(dead_time >= n_bins) does NOT reduce to first-photon-wins Coates "
        "(batch_distorted_transient_sync). It uses prob_detect instead of "
        "p_detect in the survival product. 3d's Monte-Carlo check adjudicates.",
        strict=True,
    )
...
# =========================================================================== #
# simulate_pixel_ewh is the sampled reference (Bernoulli per bin + explicit
# non-paralyzable dead time). The closed-form fast paths must match it.
MC_B = 10
MC_PULSES = 20000
...
class TestMonteCarloForwardModel:
    ...
    @pytest.mark.xfail(
        reason="Finding H1 (ground-truth side): calc_sync single-hit disagrees "
        "with the Monte-Carlo first-photon-wins result; batch_sync is correct.",
        strict=True,
    )
    ...
    def test_dt0_ground_truth_is_detection_probability(self):
        # The sampled truth at dt=0 follows detection prob (1 - e^{-phi}), NOT phi.
        phi = torch.rand(MC_B, generator=torch.Generator().manual_seed(5)) * 2.0 + 0.5
        mc = _mc(phi, free_running=True, dead_time_bins=0, n_pulses=40000)
        prob = 1.0 - torch.exp(-phi)
        prob = prob / prob.sum()
        assert torch.allclose(mc, prob, atol=0.02)

    @pytest.mark.xfail(
        reason="Finding dt=0: fast async(dt=0) returns counts ∝ arrival rate phi, "
        "but Monte-Carlo ground truth ∝ detection prob (1 - e^{-phi}). They "
        "diverge for non-flat, high-flux scenes.",
        strict=True,
    )

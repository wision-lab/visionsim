...
def plot_edh_per_pixel(config,
    ...
    for i in range(num_fovs):
        current_ax = ax2 if num_fovs == 1 else ax2[i]
        current_ax.imshow(albedo_frames.detach().cpu().numpy() * fov_masks[i].detach().cpu().numpy(), cmap="gray", vmin=0, vmax=1)
        current_ax.set_title(f"FOV {i+1}")
        current_ax.axis('off')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.show()
    ...
    for i in range(num_fovs):
        current_ax = ax3 if num_fovs == 1 else ax3[i]
        current_ax.imshow(depth_frames.detach().cpu().numpy() * fov_masks[i].detach().cpu().numpy(), cmap="viridis", vmin=0, vmax=10) # Assuming max depth of 10m based on 10.0/255.0 scaling
        current_ax.set_title(f"FOV {i+1}")
        current_ax.axis('off')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.show()
    ...
    for i in range(num_fovs):
        ...
        current_ax.bar(edh_bins[:-1], bin_height[:-1],
                       width=bin_widths, color='none', align='edge',

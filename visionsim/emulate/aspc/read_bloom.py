...
npz_files = sorted(
    glob.glob(os.path.join(npz_dir, "diff_*.npz")), key=lambda x: int(os.path.basename(x).split("_")[1].split(".")[0])
)
...
def update_display():
    ...
    im2 = ax2.imshow(zoomed_region, cmap="viridis", aspect="auto", extent=[x_start, x_end - 1, y_end - 1, y_start])
    cbar2 = plt.colorbar(im2, ax=ax2, label="Value")
    cbars.append(cbar2)
    ax2.plot(max_idx[1], max_idx[0], "r*", markersize=15, label=f"Max at ({max_idx[0]}, {max_idx[1]})")

...
def to_numpy(*tensors):
    #RY. If only one tensor
    if len(tensors) == 1:
        #RY. if it is a tensor
        if torch.is_tensor(tensors[0]):
            #RY. detach tensor from any computation graph and move to cpu
            return tensors[0].detach().cpu().numpy()
        else:
            return np.array(tensors[0])
    #RY. convert each t in tensor to np array
...
def to_tensor(*tensors, dtype=torch.float32, device=None):
    if len(tensors) == 1:
        if torch.is_tensor(tensors[0]):
            #RY. if already tensor, convert it to desired type
            return tensors[0].to(dtype=dtype, device=device)
        #RY. if not tensor, create tensor
...
def img_to_tensor(*imgs, batched=True, dtype=torch.float32, device=None):
    #RY. convert to tensor or if already tensor use as is.
    ...
    #RY. return batch of tensors stacked, or return entire list
